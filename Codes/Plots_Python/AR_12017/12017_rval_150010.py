...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug  8 20:08:10 2023
...
for i in range(0,n):
    ...
    if len(os.listdir(dir)) != 0:
        rval = np.loadtxt("PF_ext_rvals_150010_" + Time + ".dat")
...
axs[0].text(20, (cbar_max + (0.35*(cbar_max - cbar_min))), P4, fontsize=23)
axs[5].text(-60, cbar_min + 0.5*(cbar_max - cbar_min), 'Height (Mm)', rotation = 90, fontsize=18)
axs[9].text(-24, (cbar_min - (0.65*(cbar_max - cbar_min))), 'Time after ' + start_time + ' (hrs)'  + '; ($B_{th}$, $D_{sep}$) = ' + '(' + th + ',' + d + ')', rotation = 0, fontsize=18)
